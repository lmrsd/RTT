...
class RttWidget(baseType, uiType) :
	...
	def setUpConnections(self):
		...
		#
		self.iprBtn.clicked.connect(partial(self.launchIprRender))
		#Option btn bindings
		self.dofBtn.clicked.connect(partial(self.iprRenderOption,'dof'))
		#test functions
		#QtWidgets.QShortcut(QtGui.QKeySequence("t"),self, self.test)
	...
	def launchIprRender(self):
		if(self.iprBtn.isChecked()):
			cmds.vrayStartIPR()
		else:
			print('renderDone,save image...')
			#Save Image

	def iprRenderOption(self,*args):
		print (args)
		if 'dof' in args:
			if(self.dofBtn.isChecked()):
				print('change render settings')

		cmds.vrayUpdateIPR()

	def vrayImageComplete():
		print('marche?')

	def initStoredParams(self):
		vrayParams = add3DMaxRenderOption(getRenderesParams(cmds.renderers.current))
		rtt.writeParam(vrayParams)
